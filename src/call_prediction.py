...
def call_prediction_api(data):
    try:
        ...
        print(f"✅ Prédiction : {result['prediction']} ({round(result['probability']* 100, 2)}%)")
        return {
...
def predict_from_csv(csv_path, output_path="predictions.csv"):
    ...
    for index, row in df.iterrows():
        try:
            ...
            print(f"[{index+1}/{len(df)}] ✅ {result['prediction']} ({round(result['probability']*100, 2)}%)")
...
def predict_and_save_csv(csv_path, output_dir="outputs", output_name="predictions_result.csv"):
    ...
    for index, row in df.iterrows():
        ...
        result = call_prediction_api(payload)
        results.append(result)
        print(f"[{index+1}/{len(df)}] ✅ {result['prediction']} ({result['probability']}%)")
        time.sleep(0.05)
